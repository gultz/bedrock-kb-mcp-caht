...
const isValidBatchArgs = (args) => {
    return (typeof args === 'object' &&
        ...
        args.genes.every((gene) => typeof gene === 'string' && gene.length > 0) &&
        (args.format === undefined || ['json', 'tsv'].includes(args.format)) &&
        (args.columns === undefined || Array.isArray(args.columns)));
};
const isValidTissueSearchArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
const isValidSubcellularSearchArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
class ProteinAtlasServer {
    ...
    setupResourceHandlers() {
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to fetch resource: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        });
    }
    setupToolHandlers() {
    ...
    async searchProteins(query, format = 'json', columns, maxResults) {
    ...
    async fetchSubcellularLocalization(gene) {
        const columns = ['g', 'eg', 'scl', 'scml', 'scal', 'relce'];
        return this.searchProteins(gene, 'json', columns, 1);
    }
    async fetchPathologyData(gene) {
    ...
    async fetchBloodExpression(gene) {
    ...
    async fetchBrainExpression(gene) {
    ...
    async handleGetProteinByEnsembl(args) {
    ...
    async handleGetTissueExpression(args) {
        if (!isValidGeneArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene arguments');
        }
        try {
    ...
    async handleSearchByTissue(args) {
    ...
    async handleGetBloodExpression(args) {
        if (!isValidGeneArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene arguments');
        }
        try {
    ...
    async handleGetBrainExpression(args) {
        if (!isValidGeneArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene arguments');
        }
        try {
    ...
    async handleGetSubcellularLocation(args) {
        if (!isValidGeneArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene arguments');
        }
        try {
    ...
    async handleSearchBySubcellularLocation(args) {
    ...
    async handleGetPathologyData(args) {
        if (!isValidGeneArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene arguments');
        }
        try {
    ...
    async handleSearchCancerMarkers(args) {
    ...
    async handleGetAntibodyInfo(args) {
        if (!isValidGeneArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene arguments');
        }
        try {
    ...
    async handleCompareExpressionProfiles(args) {
        if (!isValidBatchArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid comparison arguments');
        }
        try {
            const expressionType = args.expressionType || 'tissue';
            const comparisons = [];
    ...
    async handleGetProteinClasses(args) {
        if (!isValidGeneArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene arguments');
        }
        try {
