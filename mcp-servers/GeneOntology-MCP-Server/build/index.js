...
const isValidAnnotationSearchArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
const isValidEnrichmentArgs = (args) => {
    return (typeof args === 'object' &&
        ...
        args.genes.length > 0 &&
        args.genes.every((gene) => typeof gene === 'string') &&
        (args.species === undefined || typeof args.species === 'string') &&
        (args.ontology === undefined || ['molecular_function', 'biological_process', 'cellular_component', 'all'].includes(args.ontology)) &&
        (args.background === undefined || (Array.isArray(args.background) && args.background.every((gene) => typeof gene === 'string'))) &&
        (args.p_value_threshold === undefined || (typeof args.p_value_threshold === 'number' && args.p_value_threshold > 0 && args.p_value_threshold <= 1)));
};
const isValidCompareTermsArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
class GeneOntologyServer {
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            if (termMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch GO term ${termId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
    ...
    async handleSearchGoTerms(args) {
        ...
        try {
            ...
            const searchResults = {
                ...
                results: response.data.results?.map((term) => ({
                    id: term.id,
                    name: term.name,
    ...
    async handleGetOntologyStats(args) {
        try {
            const stats = {
