...
const isValidDownloadArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
        typeof args.pdb_id === 'string' &&
        args.pdb_id.length === 4 &&
...
class PDBServer {
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            if (structureMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch PDB structure ${pdbId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
            ...
            if (coordinatesMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch PDB coordinates for ${pdbId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
    ...
    async handleGetStructureInfo(args) {
    ...
    async handleGetStructureQuality(args) {
        if (!isValidPDBIdArgs(args)) {
